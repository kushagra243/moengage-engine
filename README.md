...
- **Cohorts & SOPs** — monthly segment uploads are decoded from their names (HVT_Sep26 → family HVT, version 2026-09) into the Overview Cohorts card with version-over-version deltas and studies to run; the SOPs tab holds standard operating procedures per campaign type (sequence, cohort family, duration, frequency, holdout, KPI, kill rules) that run as approval-gated proposals with pre-, mid- and post-flight checks.
- **North star & guardrails** — one north-star sentence, hard per-user communication limits (channel × day/week, stage overrides, regime multipliers) that the team sets and the engine enforces in SOP checks, pre-flight and brief checks; a cohort peace index (too much / too little contact) and a daily SOP miss/breach monitor. Every programme gets a Flight Plan: the full campaign requirement document, including what is possible now versus what needs data or API access.
- **Skills** — fifteen skills under `.claude/skills/` are shared by Claude Code and the in-app agent: MoEngage product and API, this codebase, CLM doctrine, and six marketing skills for crypto spot/perps/tokenised perps (derivatives lifecycle, compliance copy by jurisdiction, event tracking plan, copywriting bank, analytics playbook, growth calendar). `CLAUDE.md` is the committed project memory, so a fresh clone on another Mac starts with full context and `./setup.sh` re-runs are instant.
