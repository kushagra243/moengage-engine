...
views.intel = async () => {
  ...
  try {
    ...
    body.innerHTML = `<div class="cards" style="margin-bottom:16px">${(d.rivals || []).length ? d.rivals.map(r => `<div class="card"><div class="nm"><span>${esc(r.name)}</span><span class="chip sev" style="color:${esc(r.color)};border-color:${esc(r.color)}">${esc(r.threat)}</span></div>
      ...
      <div class="cols"><div class="c3"><div class="panel"><div class="ph"><span class="t">Campaigns detected · 48h</span><span class="meta">announcements · blogs · news · app notes</span></div><div class="pb">${(d.campaigns || []).length ? `<div class="grid-tbl" style="grid-template-columns:80px minmax(0,1fr) 120px 80px 150px"><div class="h">venue</div><div class="h">campaign</div><div class="h">type</div><div class="h">impact</div><div class="h">our counter</div>${d.campaigns.slice(0, 14).map(c => `<div class="row" style="display:contents"><div class="r mute">${esc(c.venue)}</div><div class="r">${c.url ? `<a href="${esc(c.url)}" target="_blank" rel="noopener">${esc(c.title)}</a>` : esc(c.title)}<div class="ghost" style="font-size:10px">${esc(c.source || '')} · ${esc(c.published_at || '')}</div></div><div class="r mute" style="font-size:10px;letter-spacing:.08em">${esc(c.type)}</div><div class="r" style="color:${c.impact === 'MATERIAL' ? C.mg : c.impact === 'WATCH' ? C.am : 'var(--text-ghost)'}">${esc(c.impact)}</div><div class="r" style="font-size:10px">${c.counter_sop ? `<a href="#" data-ask-text="Counter this competitor campaign (${esc(c.venue)}: ${esc(c.title)}) with ${esc(c.counter_sop)}: check regime and limits, run_sop dry-run, two compliant variants, no venue names. ${esc(c.counter)}">${esc(c.counter_sop.replace('sop_', ''))}</a>` : '<span class="ghost">product / monitor</span>'}</div></div>`).join('')}</div>` : empty('no competitor campaigns detected in 48h', 'sources refresh every 15 minutes')}</div></div></div>
      <div class="c1"><div class="panel"><div class="ph"><span class="t">App Store · finance (IN)</span><span class="meta">rank · rating · version</span></div><div class="pb">${Object.keys(d.apps || {}).length ? Object.entries(d.apps).sort((a, b) => (a[1].rank_top_free || 999) - (b[1].rank_top_free || 999)).map(([k, a]) => `<div style="display:flex;justify-content:space-between;gap:8px;padding:5px 0;border-bottom:1px solid var(--line-soft);font-size:12px"><span style="color:${k === 'coindcx' ? C.cy : 'var(--text)'}">${esc(a.name)}<div class="ghost" style="font-size:10px">v${esc(a.version || '—')} · ${esc(a.updated || '')}${a.rank_top_free_7d_ago ? ` · was #${a.rank_top_free_7d_ago}` : ''}</div></span><span style="text-align:right"><span class="mono">${a.rank_top_free ? '#' + a.rank_top_free : '—'}</span><div class="ghost" style="font-size:10px">★ ${fmt(a.rating, 2)} · ${fmt(a.ratings_count, 0)}</div></span></div>`).join('') : empty('no app data')}</div></div></div></div>
