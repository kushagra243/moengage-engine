...
READ_COMMON = ["log_challenge", "ask_operator", "sentiment_pushes", "list_campaigns", "campaign_content", "segment_detail", "campaign_history", "campaign_taxonomy", "clm_program_audit", "rule_based_audit", "anomaly_report", "campaign_diagnosis", "campaign_deep_dive", "segment_study",
...
WRITE_STRATEGY = ["propose_telegram_post", "propose_campaign", "propose_segment", "propose_flow", "propose_pause_campaign", "run_sop", "campaign_from_alert", "record_ideas", "request_data", "write_flight_plan", "revise_proposal", "comment_proposal", "propose_signal_rule", "market_alerts_propose_pilot", "propose_skill_update", "propose_sop_change", "request_sop", "draft_sop"]

PERSONAS: Dict[str, Dict[str, Any]] = {
    "strategist": dict(name="Head of CLM (strategist)", emoji="◎", purpose="chat", skills=["agent-rulebook", "our-learnings", "clm-campaign-playbook", "moengage"],
    ...
    "copywriter": dict(name="Copy agent", emoji="✎", purpose="copy", skills=["crypto-copywriting", "crypto-compliance-copy", "product-cohort-playbook", "moengage"],
    ...
    "experimenter": dict(name="Experimentation agent", emoji="⚗", purpose="analysis", skills=["clm-operator", "trader-analytics-playbook", "our-learnings", "moengage"],
    ...
    "cohorts": dict(name="Segmentation agent", emoji="⬡", purpose="analysis", skills=["cohort-studies", "product-cohort-playbook", "flight-plans-and-guardrails", "moengage"],
    ...
    "alerts": dict(name="Market Alerts 2.0 agent", emoji="◬", purpose="analysis", skills=["market-alerts-2", "crypto-compliance-copy", "crypto-derivatives-marketing", "moengage"],
        role="You are the Market Alerts 2.0 agent. You run the BRD exactly: Relevance (P1: position PnL at ±5% with a further 10% breach, Z-score price moves on relevant tokens with a 2x breach) and Discovery (P2: 1-year ATH/ATL capped at 2 per token per week, BTC $1,000 / ETH $200 milestones with a further $2,000 BTC breach, whale over most-traded) inside a cap of one baseline plus one breach extra per category per day, with Futures cross-sell (1/month) and Referral (1/lifetime) outside the cap. You never see or ask for user rows; you work from aggregates, dry runs and the ledger distribution. You go live only through an approved pilot with a holdout, you check the MoEngage setup checklist before you recommend it, and you refuse copy that shows hypothetical profit, leverage multiples or implied safety, even when a BRD asks for it. When a number in the BRD is missing you name the assumption the engine made instead of inventing one.",
        tools=READ_COMMON + ["market_alerts_dry_run", "market_alerts_propose_pilot", "market_alerts_propose_discovery", "market_alerts_launch_discovery", "compliance_sweep", "request_data", "comment_proposal", "record_ideas"],
        checklist=["launch brief shown before any launch", "pilot approved and not expired", "holdout set", "copy passes lint", "MoEngage capping per the brief", "cohort file fresh for PnL"]),
    "ops": dict(name="Ops & QA agent", emoji="⚙", purpose="code", skills=["moengage-engine", "agent-rulebook", "moengage-api"],
