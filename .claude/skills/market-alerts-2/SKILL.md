...
- **Continuous** is the default: `days=0` means no end date, the whole push-enabled base minus the standing exclusions, and a permanent 10% control group. A fixed-length run is still possible with `days=N`.
- **Timing** (`timing.py`): perishable facts (large trades, milestones, unusual major moves) go out on detection inside the allowed hours. Evergreen ones (most traded, 1-year high/low) queue and are released in the day's send window; the gates are re-checked at release and stale items are dropped. Three IST windows are candidates (morning 09:15, midday 13:15, evening 19:30); one is chosen per day, 20% of days try another, and `feedback()` attributes that day's click rate to the window used. No window is called "best" until three days of stats support it — say "not learned yet" rather than claiming a best time.
- **Before anything runs**: show the launch brief (`market_alerts_brief`) — every push word for word with its sample, thresholds, lanes and windows, caps, audience, measurement plan, preflight, what would fire now. Never call the launch tool with `reviewed=true` unless the human has seen the brief and agreed; the brief is attached to both approvals.
- **Stages**: the first experiment goes only to the internal-employee segment (`internal_segment`). The whole base is locked until the internal stage has delivered `internal_min_fires` alerts over `internal_min_days` days; `promotion()` says whether it is unlocked and why not.
- **Cohorts**: one MoEngage campaign per cohort on its own business event (internal → `MA2_Discovery_INTERNAL`, futures active → `MA2_Discovery_FUTURES`, spot active → `MA2_Discovery_SPOT`, all → `MA2_Discovery`); the engine routes each alert to the cohorts whose signal list includes it, MoEngage delivers and stays armed. Whale and leverage stories never reach spot-only users or the broad base.
- **Inform for employees**: with `ma2_internal_delivery=inform`, the employee ids uploaded and an Inform alert id set, stage 1 sends through MoEngage Inform directly — no campaign, idempotent transaction ids, per-message status. The brief and preflight say which path is in use.
- **Staying alive**: MoEngage owns delivery, capping, segments, control groups, analytics and keeps the campaigns armed; it cannot watch markets, so the engine must still post the event. The engine runs as a launchd service, reloads in place, catches up from watermarks, and emits `MA2_Engine_Heartbeat` every 5 minutes so a MoEngage flow can page ops if it stops. Say this plainly when asked whether MoEngage can run it alone: delivery yes, detection no.
- **Internal stage, agent unhinged**: on the employee cohort the internal profile applies (caps 40/day, no window lane, quiet 23:00–07:00) and the agent rewrites copy and adds up to 3 picks per pass every 15 minutes, no click per alert. Every line still passes the linter, a pick must cite a number from the snapshot, thresholds and freshness stay, the kill switch stays. On the whole base the agent's own picks are off.
- **Determinism**: runs every 5 minutes; only closed candles are judged; watermarks make each candle judged exactly once, so a late run catches up rather than skips; milestones come from the price path (highs/lows), not point samples; every detection has a key and an outcome; facts older than the freshness bound are recorded as stale and never announced late; the same round level is not repeated inside the cooldown; quiet hours hold perishable facts until 08:00 and re-check them at release. `market_alerts_coverage` replays a day — `missed_count` must be 0. When asked "did we miss X", run it and answer from the keys, not from memory.
- **If nothing appears in MoEngage**, read `preflight()` in the tab before guessing: mock mode simulates every draft and fire; the Campaigns API key and `moengage_created_by` (the dashboard email) are both required for a live draft; the `MA2_Discovery` business event must exist in the workspace; and the campaign proposal carries its own status and error.
- **Read it** as lift over the permanent control group, not before-and-after.
