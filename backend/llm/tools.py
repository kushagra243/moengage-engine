...
def market_alerts_coverage(day: Optional[str] = None) -> Dict[str, Any]:
    """Prove the discovery alerts missed nothing on a day: replay closed candles and compare with what the live path recorded (expected, recorded, missed with keys, outcomes)."""
    from ..alerts2 import discovery
    c = discovery.coverage(day)
    c["missed"] = c["missed"][:20]
    return c
...
def market_alerts_discovery(dry_run: bool = False) -> Dict[str, Any]:
    ...
    from ..alerts2 import discovery
    st = discovery.status()
    out = {k: st[k] for k in ("live", "why_not_live", "experiment", "today", "caps", "whale_source", "event", "setup", "stage", "promotion", "cohorts", "keepalive")}
...
def market_alerts_brief(days: int = 0, signals: Optional[List[str]] = None, audience: str = "", control_pct: int = 10) -> Dict[str, Any]:
    """The launch brief for the discovery alerts experiment, read BEFORE launching: every push word for word with a rendered sample and its compliance result, when each signal fires and on what thresholds, the lanes and send windows, the caps, the audience and control group, the exact MoEngage draft, the measurement plan, the preflight and what would fire this minute."""
    from ..alerts2 import brief
    b = brief.build(days, signals, audience, control_pct, with_dry_run=True)
    b.pop("moengage_draft", None)
    b["markdown_available"] = "GET /api/alerts2/discovery/brief?format=md|docx"
    return b


def market_alerts_launch_discovery(audience: str = "", days: int = 0, signals: Optional[List[str]] = None, control_pct: int = 10, kpi: str = "sessions_per_week", reviewed: bool = False, cohort_ids: Optional[List[str]] = None) -> Dict[str, Any]:
    """Launch the discovery alerts experiment. Without reviewed=true it returns the full launch brief and creates nothing; with reviewed=true (only after the brief has been shown to the human and they agreed) it queues the MoEngage business-event push campaign and the engine's permission to fire. Both still need a human approval on the Ideas board."""
    from ..alerts2 import discovery
    r = discovery.launch(audience, days, signals, control_pct, kpi, created_by="agent", reviewed=reviewed, cohort_ids=cohort_ids)
    if r.get("brief"):
        r["brief"] = {k: r["brief"].get(k) for k in ("summary", "signals", "copy", "timing", "governance", "audience", "experiment", "would_fire_now", "preflight", "copy_blocking")}
    r.pop("brief_markdown", None)
    return r
...
TOOL_SCHEMAS: List[Dict[str, Any]] = [
    ...
    _fn("market_alerts_coverage", "Replay a day's closed candles and report any discovery detection the live path missed (must be zero).", {"day": STR}),
    ...
    _fn("market_alerts_brief", "The launch brief to read BEFORE launching discovery alerts: every push word for word, thresholds, lanes and windows, caps, audience, measurement plan, preflight, what would fire now.",
        {"days": {"type": "integer"}, "signals": {"type": "array", "items": STR}, "audience": STR, "control_pct": {"type": "integer"}}),
    _fn("market_alerts_launch_discovery", "Launch the discovery experiment. reviewed=false returns the brief and creates nothing; reviewed=true (only after the human has seen the brief) queues the MoEngage campaign draft and the firing permission for approval.",
        {"audience": STR, "days": {"type": "integer"}, "signals": {"type": "array", "items": STR}, "control_pct": {"type": "integer"}, "kpi": STR, "reviewed": {"type": "boolean"},
         "cohort_ids": {"type": "array", "items": STR, "description": "internal | futures_active | spot_active | all; one MoEngage campaign per cohort on its own business event"}}),
