...
LIBRARY: List[Dict[str, Any]] = [
    ...
    dict(id="funding_rate_education", title="Funding-rate 'you are paying to stay long' nudges", category="risk",
         ...
         source="Exchange-native data (perp venue funding/OI — data only, never in copy); risk-nudge pattern from Zerodha Kite", source_url="https://zerodha.com/z-connect/updates/kite-nudge-to-warn-you-about-portfolio-concentration",
    ...
    dict(id="liquidation_recovery_flow", title="Liquidation-recovery flow (T+0 silence, T+24h explainer, T+14d spot path)", category="retention",
         what="Event-triggered flow on position_liquidated: no promos for 14d; a service card immediately; a plain-language email at 24h; opt-in risk education; a no-leverage path at day 14.",
         why="Liquidated users churn at 3–5× the base rate in the following month and the first 48 hours decide it; exchanges that treat it as a service moment (Kraken, Binance risk notices) retain more of them.",
         source="Exchange risk-notice patterns; derivatives retention practice", source_url="https://www.binance.com/en/support/faq/detail/360033525271",
         how="Flow: trigger position_liquidated → cohort-sync into liquidated_14d suppression → wait 24h → email with their numbers + calculator → conditional education series → day 14 spot-first campaign with 20% holdout.",
         prereqs=["position_liquidated event", "suppression segment used as exclusion everywhere"], kpi="return_to_trade_rate_30d vs holdout; repeat liquidation 30d (guardrail)", effort="medium", regimes=["all"], channels=["in-app", "email", "whatsapp"]),
    dict(id="oi_crowding_nudge", title="Open-interest crowding nudge to position holders", category="risk",
         what="When OI on a perp rises ≥30% in 24h while price is flat, tell holders of that position that positioning is crowded and offer a margin-buffer review.",
         why="OI build without price movement precedes sharp moves; users warned about crowding size down and get liquidated less (the engine computes oi_movers daily).",
         source="Exchange-native OI (Hyperliquid metaAndAssetCtxs); risk-nudge pattern", source_url="https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/info-endpoint/perpetuals",
         how="Business event oi_surge {symbol,pct} → BE-triggered in-app to segment 'open position in symbol'; push only to leverage ≥5x; TTL 4h; 1/day; excluded: liquidated-14d.",
         prereqs=["position_opened/closed events with symbol"], kpi="risk-tool opens; positions reduced within 24h vs holdout", effort="low", regimes=["all"], channels=["in-app", "push"]),
    dict(id="tokenised_after_hours", title="'Markets that never close' — tokenised equities after US hours", category="activation",
         what="When a tokenised equity/index perp on Hyperliquid moves ≥3% while US markets are closed, educate watchers that it trades 24/7 here; weekend edition on Saturdays.",
         why="24/7 access to NVDA/SPX/gold is the differentiated feature of builder-dex perps; after-hours moves are the moment the feature explains itself.",
         source="Hyperliquid builder dexes (xyz/flx/km) listings; product differentiation", source_url="https://hyperliquid.gitbook.io/hyperliquid-docs/hyperliquid-improvement-proposals-hips/hip-3-builder-deployed-perpetuals",
         how="Hook mover_equities → segment 'watchlist contains symbol OR traded any builder dex' → push at 08:00 or 18:00 IST with the move and 'on CoinDCX' wording (never name the liquidity venue); product_education angle; never 'trade earnings'.",
         prereqs=["watchlist events with asset_class", "dexes_traded attribute"], kpi="tokenised weekly actives; first builder-dex fill rate (holdout 20%)", effort="low", regimes=["trending_up", "chop", "high_volatility_up"], channels=["push", "in-app"]),
    dict(id="perp_intent_education", title="Perp intent without a trade → education path (not a promo)", category="activation",
         what="Users who viewed a perp market ≥2 times in 14 days but never traded one get a 3-step in-app/email education path: what a perp is, funding, isolated margin — with an explicit 'spot is fine' exit.",
         why="Intent is the only acceptable trigger for derivatives content; educated first-perp users have lower 30-day liquidation rates, and the exit option keeps compliance posture clean.",
         source="Regulatory-safe derivatives onboarding practice (FCA appropriateness logic; ASCI VDA rules)", source_url="https://www.fca.org.uk/publications/policy-statements/ps23-6-financial-promotion-rules-cryptoassets",
         how="Segment: market_viewed{product=perp} ≥2 in 14d AND order_filled{product=perp}=0 AND country not UK/US → flow with 3 education messages 48h apart; KPI measured on opt-in cohort; holdout 20%.",
         prereqs=["market_viewed with product attribute", "country attribute"], kpi="first_perp_trade_rate_30d (opt-in) with liquidation_rate_30d guardrail", effort="medium", regimes=["trending_up", "chop"], channels=["in-app", "email"]),
    dict(id="funding_hour_digest", title="Pre-funding-window position digest (13:00 IST)", category="engagement",
         what="30 minutes before the 08:00 UTC funding reference, a Cards/in-app digest of the user's open perps: funding cost per day, distance to liquidation, margin mode.",
         why="Position reviews cluster around funding windows; a persistent card at the right moment replaces several anxious app opens and reduces support load.",
         source="Hyperliquid hourly funding mechanics; MoEngage Cards + Content APIs", source_url="https://www.moengage.com/docs/user-guide/campaigns-and-channels/getting-started/campaign-content/content-apis",
         how="Content API /positions?uid= served by the engine (cached 60s) → Cards campaign daily 13:00 IST to 'open perp position' segment; push only if distance to liquidation < 10%.",
         prereqs=["Content API endpoint", "position attributes"], kpi="card opens; liquidation rate of digest users vs holdout", effort="medium", regimes=["all"], channels=["cards", "in-app", "push"]),
    dict(id="macro_print_freeze", title="Macro-print freeze and T−24h risk brief", category="risk",
         what="For CPI/NFP/FOMC: T−24h risk brief to leverage users, freeze market-linked promotional sends T−2h → T+2h, factual recap after.",
         why="Volatility around US prints liquidates leveraged Indian users at night IST; the freeze prevents tone-deaf sends and the brief measurably raises SL usage.",
         source="Engine calendar (ForexFactory/Finnhub high-impact events)", source_url="https://www.forexfactory.com/calendar",
         how="Business event macro_event_soon → BE-triggered in-app to leverage ≥3x users; scheduler rule blocks ride_the_market missions in the window; autopilot protect handles regime flips.",
         prereqs=["calendar source (built in)"], kpi="SL/TP set rate T−24h→T; liquidations on print nights vs prior prints", effort="low", regimes=["all"], channels=["in-app", "push"]),
]
